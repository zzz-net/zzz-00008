...
def create_batch(data, operator):
    ...
    if invalid_tickets:
        return None, '；'.join(invalid_tickets)
...
def confirm_batch(batch_id, receiver_person):
    batch = HandoverBatch.query.get(batch_id)
    if not batch:
        return None, '交接批次不存在'

    if batch.status == 'confirmed':
        return None, '该交接批次已经确认过，不能重复确认'

    if batch.status == 'returned':
        return None, '该交接批次已被退回，需要重新创建交接'

    if batch.status != 'pending':
        return None, f'当前批次状态为 {batch.status}，无法确认'
...
def return_batch(batch_id, receiver_person, reason):
    batch = HandoverBatch.query.get(batch_id)
    if not batch:
        return None, '交接批次不存在'

    if batch.status == 'returned':
        return None, '该交接批次已经被退回，不能重复退回'

    if batch.status == 'confirmed':
        return None, '该交接批次已确认，无法退回'

    if batch.status != 'pending':
        return None, f'当前批次状态为 {batch.status}，无法退回'
